...
**Список свойств**  
activeEnergyExport: Активная энергия, экспортируемая за фазу с момента начала измерения. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
activeEnergyImport: Активная энергия, импортируемая, т.е. потребляемая за фазу с момента начала дозирования. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
activePower: Фактические значения будут вызывать подсвойства, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
address:   
alternateName:   
apparentEnergyExport: Очевидная энергия, экспортируемая за фазу с момента начала замера. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
apparentEnergyImport: Очевидная энергия, импортированная, т.е. потребленная за фазу с момента начала учета. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
apparentPower: Очевидная потребляемая мощность в каждой фазе. Фактические значения будут передаваться подсвойствами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3  
areaServed:   
current: Электрический ток. Фактические значения будут передаваться одной под-свойством на фазу переменного тока и нейтральный провод: L1, L2, L3 и N.  
dataProvider:   
dateCreated:   
dateEnergyMeteringStarted: Дата начала учета энергии.  
dateModified:   
description:   
displacementPowerFactor: Коэффициент мощности смещения для каждой фазы. Количество основано на фундаментальной частоте системы. Фактические значения будут передаваться одной под-свойством на фазу переменного тока: L1, L2 и L3  
frequency: Частота схемы.  
id:   
location:   
name:   
owner:   
phaseToPhaseVoltage: Напряжение между фазами. Значение для каждой пары фаз: фазы 1 и 2 (L12), фазы 2 и 3 (L32), фазы 3 и 1 (L31).  
phaseVoltage: Напряжение между каждой фазой и нейтральным проводником. Фактические значения будут передаваться одной под-свойством на фазу переменного тока: L1, L2 и L3  
powerFactor: Коэффициент мощности для каждой фазы. Фактические значения будут передаваться одной под-свойством на фазу переменного тока: L1, L2 и L3  
reactiveEnergyExport: Фундаментальная частотная реактивная энергия, экспортируемая по фазам с момента начала измерения. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
reactiveEnergyImport: Фундаментальная частотная реактивная энергия, импортируемая, т.е. потребляемая за фазу с момента начала дозирования. Фактические значения будут передаваться подкомпонентами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
reactivePower: Фундаментальная частотная реактивная мощность. Фактические значения будут передаваться подмножествами, названия которых будут равны названиям каждой из фаз переменного тока: L1, L2, L3.  
refDevice: Отношения. Устройство(и), используемое(ые) для получения измерения.  
refTargetDevice: Отношения. Устройство(и), для которого(ых) было произведено измерение.  
seeAlso:   
source:   
thdCurrent: Полное гармоническое искажение электрического тока. Фактические значения будут передаваться одной под-свойством на фазу переменного тока: L1, L2 и L3  
thdVoltage: Полное гармоническое искажение напряжения для каждой фазы. Фактические значения будут передаваться одной под-свойством на фазу переменного тока: L1, L2 и L3  
totalActiveEnergyExport: Общий объем экспортируемой энергии с момента начала замера (с момента `датыЕгіегМетгіегМетгіс "тагтіоп").  
totalActiveEnergyImport: Суммарная импортируемая энергия, т.е. потребленная с момента начала замера (с момента `датыEnergyMeteringStarted`).  
totalActivePower: Активная потребляемая мощность (подсчет всех фаз)  
totalApparentEnergyExport: Суммарная экспортируемая энергия (в пересчете на кажущуюся мощность) с момента начала замера ("Дата начала замера").  
totalApparentEnergyImport: Суммарная импортированная энергия, т.е. потребленная (с учетом кажущейся мощности) с момента начала замера (`датаЕnergyMeteringStarted`).  
totalApparentPower: Очевидная потребляемая мощность (считая все фазы).  
totalDisplacementPowerFactor: Коэффициент мощности смещения, включая все фазы. Количество основано на фундаментальной частоте системы  
totalPowerFactor: Коэффициент мощности, включая все фазы  
totalReactiveEnergyExport: Суммарная номинальная частотная реактивная энергия, экспортируемая с момента начала измерения (с момента `dateEnergyMeteringStarted`).  
totalReactiveEnergyImport: Суммарная импортируемая, т.е. потребленная энергия (с учетом реактивной мощности основной частоты) с момента начала замера (`датаЕnergyMeteringStarted`).  
totalReactivePower: Потребляемая реактивная мощность (подсчет всех фаз)  
type: Это должно быть равнозначно `Трехфазовому методу измерений`.

Текст для включения между общим названием и описанием.  
  
Глобальное описание: **Электрические измерения от системы, использующей трехфазный переменный ток.
## Данные Модель описания свойств


Заказано в алфавитном порядке
...
Текст для включения после списка свойств
